...
class HGNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.plm_config = AutoConfig.from_pretrained(args.plm_mode)
        self.encoder = AutoModel.from_pretrained(args.plm_mode)
        self.tokenizer = AutoTokenizer.from_pretrained(args.plm_mode)
        self.pad = self.tokenizer.vocab["[PAD]"]
        self.d_h_tmp = self.plm_config.hidden_size

        self.instance_aff =  nn.Linear(self.d_h_tmp, args.d_h)
        ...
        self.v_dec_act_aff = nn.Linear(args.d_h, args.d_h, bias=False)
        self.e_dec_act_aff = nn.Linear(args.d_h, args.d_h, bias=False)
    ...
    def encode_instance(self, ins, ins_lens, ins_classes, ins_sids):
        ...
        # ins_vec:          (n_ins, d_h)
        ins_enc, ins_vec = self.encode(ins, ins_lens)

        ins_vec = self.instance_aff(self.dropout(ins_vec))
    ...
    def generate(self, sample, max_beam_size=5):
        """
        ...
        while not check_schedule(v_schedules, is_finished, v_n_beams):
            ...
            while not check_schedule(e_schedules, is_finished, e_n_beams):
                ...
                if self.args.use_eg:
                    update_schedule_with_eg(schedules=e_schedules,
                                                       in_beams=ein_beams,
    ...
    def outlining_with_beam_search(self, context, q_enc, q_mask, max_beam_sz, data):
        ...
        while not is_complete(completed_beams, max_beam_sz) and t < self.args.max_n_step:
    ...
    def encode(self, src, src_lens, src_segment=None):
        """
        ...
        if src_segment is None:
            src_segment = torch.zeros_like(src)
            if src.cuda:
                src_segment = src_segment.to(src.device)

        src_mask = length_array_to_mask_tensor(src_lens, reverse=False)
        if src.cuda:
            src_mask = src_mask.to(src.device)
        output = self.encoder(input_ids=src,
                              token_type_ids=src_segment,
                              attention_mask=src_mask)
        src_enc = output["last_hidden_state"]
        src_vec = output["pooler_output"]
        return src_enc, src_vec
