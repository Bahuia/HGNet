...
if __name__ == '__main__':
    ...
    train_loader = AQGGenerationDataLoader(args)
    ...
    valid_loader = AQGGenerationDataLoader(args)
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        for i, b in enumerate(train_loader.next_batch()):
            data = b[-1]
            gold_aqg_obj_labels, gold_v_instance_obj_labels, gold_e_instance_obj_labels = b[-7:-4]

            optimizer.zero_grad()
            loss, action_probs, v_instance_action_probs, e_instance_action_probs = model(b)
            ...
            for s_id in range(len(gold_aqg_obj_labels)):
                is_aqg_correct = True
                for j in range(len(gold_aqg_obj_labels[s_id])):
                    pred_obj = torch.argmax(action_probs[s_id][j], dim=-1).item()
                    if pred_obj == gold_aqg_obj_labels[s_id][j]:
                        n_aqg_step_correct += 1
                    else:
                        is_aqg_correct = False
                n_aqg_step_total += len(gold_aqg_obj_labels[s_id])
                n_aqg_correct += is_aqg_correct

                is_v_correct = True
                gold_labels = [x for x in gold_v_instance_obj_labels[s_id] if x != -1]
                for j in range(len(gold_labels)):
                    pred_obj = torch.argmax(v_instance_action_probs[s_id][j], dim=-1).item()
                    if pred_obj == gold_labels[j]:
                        n_v_step_correct += 1
                    else:
                        is_v_correct = False
                n_v_step_total += len(gold_labels)

                is_e_correct = True
                for j in range(len(gold_e_instance_obj_labels[s_id])):
                    pred_obj = torch.argmax(e_instance_action_probs[s_id][j], dim=-1).item()
                    if pred_obj == gold_e_instance_obj_labels[s_id][j]:
                        n_e_step_correct += 1
                    else:
                        is_e_correct = False
                n_e_step_total += len(gold_e_instance_obj_labels[s_id])
        ...
        for b in valid_loader.next_batch():
            ...
            data = b[-1][0]
            pred_aqgs = model.generate(b, beam_size=args.beam_size, sparql_cache=sparql_cache)
